...
public class BookingAdapter extends ArrayAdapter<BookingHelper> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if(resource == R.layout.booking_admin_card){
            TextView carOwner = view.findViewById(R.id.user_name);
            TextView ownerNumber = view.findViewById(R.id.user_phoneNo);

            User.getUserById(bookingHelper.getReceiverId(), new UserDao() {
                @Override
                public void getUser(User user) {
                    UserDao.super.getUser(user);

                    carOwner.setText(user.getFullName());
                    ownerNumber.setText(user.getPhoneNumber());
                }
            });
        }
